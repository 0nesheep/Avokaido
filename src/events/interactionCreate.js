...
module.exports = {
  ...
  async execute(interaction) {
    ...
    var currShop = await shopModel.findOne({ id: shopId });
    ...
    if (currShop) {
      ...
      try {
        ...
        if (interaction.customId == 'shopClaim') {
          const currUser = await profileModel.findOne({ userId: user.id });
          if (claimedUsers.includes(user.id)) {
            return await user.send(
              'You have already purchased from the shop! Save some for others >:('
            );
          } else if (!currUser) {
            return await user.send(
              'You have not registered! Please register before purchasing from the shops!'
            );
          }
          ...
          if (balanceAmt < price) {
            return await user.send(
              `You do not have enough plums to purchase the item from **"${title}"** >:0`
            );
          }
          ...
          const newShopData = await shopModel.findOne({ id: shopId });
          ...
          return await user.send({
            ...
            files: ['./src/images/thank_you.png']
          });
        } else if (interaction.customId == 'shopClose') {
          if (
            !interaction.member.roles.cache.has('648704466614353930') &&
            !interaction.member.roles.cache.has('649480335498805249')
          ) {
        ...
        console.log(
          `Error interacting with shop for user ${user.id}: ` + e.message
        );
      }
    } else {
      currShop = await decorModel.findOne({ id: shopId });
      if (currShop) {
        const price = currShop.price;
        ...
        try {
          ...
          if (interaction.customId == 'shopClaim') {
            ...
            const newShopData = await decorModel.findOne({ id: shopId });
            ...
            return await user.send({
              content: `You have claimed the decoration from ${title}!`,
              files: ['./src/images/thank_you.png']
            });
          } else if (interaction.customId == 'shopClose') {
            if (
