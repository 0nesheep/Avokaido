...
module.exports = {
  ...
  async execute(message) {
    ...
    if (
      message.channel.id != id.botChannel &&
      //bot testing channel
    ...
    if (command == 'help') {
      ...
      if (!currUserData || currUserData.nickname == null) {
        return message.reply(
          'Please register with the bot first by using `!setname <name>`'
        );
      } else {
        message.client.commands
          .get('card')
          .execute(message, target, image, hehe);
      }
      ...
      if (!currUserData || currUserData.nickname == null) {
        ...
        message.client.commands
          .get('give')
          .execute(message, messageArray, target, image, hehe);
      }
      ...
      if (currUserData.nickname == null) {
        ...
        message.client.commands
          .get('daily')
          .execute(message, dailyCooldown, image, hehe);
      }
    ...
    if (command == 'add') {
      ...
      try {
        ...
        message.client.commands
          .get('permashop')
          .execute(message, price, link, image, hehe);
      } catch (error) {
      ...
      try {
        ...
        message.client.commands
          .get('decorshop')
          .execute(message, price, achievement, image, hehe);
      } catch (error) {
        console.log(
          'Error occurred with make decoration permanent shop command',
          error
        );
      }
      ...
      try {
        message.client.commands
          .get('editTitle')
          .execute(message, messageArray, image, hehe);
      } catch (e) {
      ...
      try {
        ...
        console.log('Error occurred with raffle command: ' + error.message);
      }
    }
  }
};
