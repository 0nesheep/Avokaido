...
async function generateCard(currUserData, message) {
  try {
    const currUser = await profileSchema.findOne({ userId: message.author.id });
  ...
  try {
    styleRef = await achModel.findOne({ index: cardData.decor });

    fgRef = await achModel.findOne({ index: fgRef });

    special = await achModel.findOne({ index: special });
  } catch (e) {
  ...
  try {
    baseRef = await achModel.findOne({ index: cardData.decor });
  } catch (e) {
  ...
  if (typeof styleRef === 'object') {
    if (styleRef == null) {
      ...
      styleRef.style.bgTop.imagePath != undefined &&
      currUserData.ach[styleRef.index]
    ) {
  ...
  for (let i = 0; i < achArray.length; i++) {
    let badgeRef;
    try {
      badgeRef = await achModel.findOne({ index: achArray[i] });
    } catch (e) {
  ...
  if (currUserData.memories > 0) {
    ...
    const memory = await readFile(
      `./src/images/mem${currUserData.memories}.png`
    );
  ...
  if (typeof fgRef == 'object') {
    if (
      fgRef.fg.fgTop.imagePath != undefined &&
      currUserData.ach[fgRef.index]
    ) {
      const info = fgRef.fg;
...
Use the edit card command to check it out!`);
  ...
  if (rand1 < 0.02) {
    ...
    message.reply({
      content:
        'Feesh spent a lot of effort making cards possible for you...please show her some love!',
...
Use the edit card command to check it out!`);
  ...
  if (rand4 < 0.02) {
    ...
    message.reply({
      content: 'Hehe..hehe..he..help',
      allowedMentions: { repliedUser: false }
    });
    setTimeout(
      () =>
        message.reply({
          content: 'Hehe floats away...',
          allowedMentions: { repliedUser: false }
        }),
      7000
    );
