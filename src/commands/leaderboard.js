...
module.exports = {
    ...
    async execute(message, image, hehe) {
        ...
        let lvlData;
        try {
            lvlData = await profileModel.find({nickname: {$ne: null}}).sort( { level: -1, dream: -1 } ).skip(pos).limit(10);
        } catch(e) {
        ...
        let lbEmbed;
        if (lvlData) {
            const text = lvlData.map((obj, position) => `${position + pos + 1}. **${obj.nickname}**: Level ${obj.level}`
            ).join('\n');
            ...
            lbEmbed = new EmbedBuilder()
                ...
                .setThumbnail(image)
                .addFields(
                    { name: "Dream Leaderboard", value: text}
                )
                .setFooter({text: hehe});
        }

        const msg = await message.channel.send({embeds: [lbEmbed], components: [lbButtons]});
        ...
        collector.on('collect', async interaction => {
            ...
            if (interaction.customId == 'next' ) {
                ...
                if (lvlData) {
                    const newText = lvlData.map((obj, position) => `${position + pos + 1}. **${obj.nickname}**: Level ${obj.level}`
                    ).join('\n');
                    ...
                    lbEmbed = new EmbedBuilder()
                        ...
                        .setThumbnail(image)
                        .addFields(
                            { name: "Dream Leaderboard", value: newText}
                        )
                        .setFooter({text: hehe});
                        
                    }
                    if (pos + lvlData.length >= await profileModel.countDocuments({nickname: {$ne: null}})) {
                    ...
                    if (lvlData) {
                        ...
                        lbButtons = new ActionRowBuilder()
                            ...
                            try {
                                lvlData = await profileModel.find({nickname: {$ne: null}}).sort( { petals: -1 } ).skip(pos).limit(10);
                            } catch(e) {
                            ...
                            if (lvlData) {
                                ...
                                lbEmbed = new EmbedBuilder()
                                    ...
                                    .setThumbnail(image)
                                    .addFields(
                                        { name: "Petals Leaderboard", value: newText}
                                    )
                                    .setFooter({text: hehe});
