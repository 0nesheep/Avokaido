...
module.exports = {
  ...
  async execute(message, target, image, hehe) {
    ...
    if (!message.member.roles.cache.has(id.seed)) {
      let seed = await message.member.guild.roles.cache.find(
        (role) => role.id === `${id.seed}`
      );
    ...
    const myEmbed = new EmbedBuilder()
      ...
      .setThumbnail(image)
      .setDescription(
        `You have ${currUserData.petals} ${emotes.plum}
...
Level up to level 2 to get access to the profile card!`
    ...
    const attach = new AttachmentBuilder(await canvas.encode('png'), {
      name: 'card.png'
    });
