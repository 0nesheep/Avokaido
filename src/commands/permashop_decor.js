const shopModel = require('../models/decorShop.js');
const {
  EmbedBuilder,
  ActionRowBuilder,
  ButtonBuilder,
...
module.exports = {
  name: 'decorshop',
  async execute(message, price, achievement, image, hehe) {
    if (price == undefined || achievement == undefined) {
      return message.reply('Please enter the price and achievement!');
    }
    if (isNaN(price) || isNaN(achievement)) {
      return message.reply('Both price and achievement should be numbers');
    }

    if (price < 0 || achievement < 0) {
    ...
    var title;
    const titleAsk = await message.reply(
      'Please reply with the message/title of the shop'
    );
    ...
    titleCollector.on('collect', async (m) => {
      ...
      const confirmMessage = new EmbedBuilder()
        ...
        .setThumbnail(image)
        .setDescription(
          `Are you sure you want to make a permanent shop with:
            Price: ${price} ${emotes.plum}, achievement ${achievement}, title/message ${title}`
        )
        .setFooter({ text: hehe });
      ...
      const msg = await titleMsg.reply({
      ...
      **Price:** ${price} ${emotes.plum}`)
      ...
      confirmCollector.on('collect', async (interaction) => {
        ...
        if (interaction.customId == 'confirm') {
          ...
          //send to channel
          const sentMsg = await message.client.channels.cache
            .get(id.permShopChannel)
            .send({
          ...
          msg.edit({
            content: 'You have successfully created the shop!',
            embeds: [],
            components: []
          });
          ...
          interaction.deferUpdate();
          msg.edit({
            content: 'Slowly backs away...hehe',
            embeds: [],
            components: []
          });
        }
