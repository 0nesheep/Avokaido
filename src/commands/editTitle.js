...
module.exports = {
  ...
  async execute(message, messageArray, image, hehe) {
    const permashopChannel = await message.client.channels.cache.get(
      id.permShopChannel
    );

    if (!isNaN(Number(messageArray[1]))) {
      const editMessage = await permashopChannel.messages.fetch(
        messageArray[1]
      );
      if (messageArray[2] == undefined) {
        try {
          ...
          const shop = await shopModel.findOne({ id: messageArray[1] });
