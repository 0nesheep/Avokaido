...
module.exports = {
    ...
    async execute(message, target) {
        ...
        var hasAttacked = await submitSchema.findOne(
        ...
        if (currUserData) {
            if (!hasSubmitted) {
                try {
                    ...
                    console.log(`Error adding petals to artfight event to ${message.author.id}: ` + e.message);
                    const currDate = new Date();
        ...
        let galMsg
        try {
            galMsg = await gallery.send({ files: [message.attachments.first()], content: `By <@${message.author.id}> to <@${target.id}>` });
        ...
        try {
            ...
            console.log(`Error creating af submission for <@${message.author.id}> : ` + e.message);
