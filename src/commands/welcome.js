...
module.exports = {
  ...
  async execute(message, authorId, image, hehe) {
    ...
    let welcomeMessage;
    try {
      welcomeChannel = await message.guild.channels.cache.get(
        id.welcomeChannel
      );
    ...
    //assign roles
    try {
      let verified = await message.guild.roles.cache.find(
        (role) => role.name === `${id.verifiedRole}`
      );
      let unverified = await message.guild.roles.cache.find(
        (role) => role.id === `${id.notverified}`
      );
      ...
      const date = new Date();
      console.log(
        `Error assigning verified role to ${message.author} at ${date.toString()}: ` +
          e.message
      );
    }
  }
