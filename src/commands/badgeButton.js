...
async function generateAvailText(achArray) {
  ...
  for (let i = 1; i < achArray.length; i++) {
    if (achArray[i]) {
      let achData;
      try {
        achData = await achModel.findOne({ index: i });
      } catch (e) {
        console.log(
          'Error getting achievement when generating avail badges: ' + e.message
        );
        continue;
      }
...
module.exports = {
  ...
  async execute(
    ...
    const badgeEmbed = new EmbedBuilder()
      ...
      .setThumbnail(image)
      .setDescription(
        `Reply to this message to set your badges!
...
Please reply to this message within 3 minutes!`
    ...
    msg.edit({
      embeds: [badgeEmbed],
      components: [clearButton, backButton],
      file: [attach]
    });
    ...
    collector.on('collect', async (m) => {
      ...
      if (check(m)) {
        ...
        for (let i = 0; i < 5 && i < badgesA.length; i++) {
          ...
          if (isNaN(badgesA[i])) {
            m.reply(
              'Please only reply the *numbers* corresponding to your badges!'
            );
            break;
          } else if (!achArray[badgesA[i]]) {
            m.reply(
              `You do not have achievement ${badgesA[i]}! Please try again.`
            );
            break;
          } else if (data.includes(badgesA[i])) {
            m.reply(
              `You have entered a duplicate badge number! The duplicate badge is ${badgesA[i]}.`
            );
            break;
          } else {
            data.push(badgesA[i]);
