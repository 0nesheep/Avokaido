    ...
    if (num <= 10 && !message.content.includes('<@')) { //change to 5
      ...
      if (rep.includes('avokaido')) {
        ...
        if (author == 596346169962135563) {
          ...
          message.channel.send("Y..you... No you're not! You're lying! :anger:");
        }
        ...
        const replies = ["! I'm AvoKaido", '! Nice to meet you!', '~ I am AvoKaido','! How are you today?'];
        const random = Math.floor(Math.random() * replies.length);
        message.channel.send("Hi " + rep + replies[random]);
      }

      function afterIm(array) {
          if (array[0] == "i'm" || array[0] == "am" ||
              array[0] == "im" || array[0] == "I'm" ||
              array[0] == "Im") {
