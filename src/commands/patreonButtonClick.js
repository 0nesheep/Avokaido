...
module.exports = {
    ...
    async execute(message, interaction, msg, achArray, attach, backButton, image, hehe) {
        ...
        const patreonEmbed = new EmbedBuilder()
            ...
            .setThumbnail(image)
            .setDescription(`What would you like to change your decor to?
Do check out the Encyclopedia if you need a reference!`                                                                                                                                                                                                                                                 )
        ...
        try {
            const patreonData = await specialModel.findOne(
            { index: 20 }
            );
        ...
        if (total != 0) {
            msg.edit({ embeds: [patreonEmbed], components: buttonArray, files: [attach] });
        } else {
            foreground.setDescription("You do not have any available foregrounds!");
            msg.edit({ embeds: [patreonEmbed], components: [returnToNonSpec], files: [attach]});
        }
    }
}