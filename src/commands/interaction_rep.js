    ...
    execute(message) {
        ...
        if (contArray.length !== 1) {
            if (bad.includes(contArray[0].toLowerCase())) {
                ...
                const reply =['AvoKaido is recording the crime','AvoKaido revives ' + subject, 
            ...
            } else if (contArray[1].toLowerCase() == 'kai' || contArray[1].toLowerCase() == 'avokaido') {
                message.channel.send('### ' + contArray[0].split("").join(" ").toUpperCase() + '   M Y   O W N   S E L F');

            } else {
                const reply = ['Avokaido also ' + contArray[0] + ' ' + contArray[1], 
                    '*Heavily ' + contArray[0] + ' ' + contArray[1] + '*','*' + contArray[0] + ' ' + contArray[1] + ' multiple times*'];
                const rand = Math.floor(Math.random() * reply.length);
