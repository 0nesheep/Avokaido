...
module.exports = {
  ...
  async execute(message, image, hehe) {
    ...
    let attach = new AttachmentBuilder(await canvas.encode('png'), {
      name: 'card.png'
    });
    ...
    if (cardData.special != 0) {
      msg = await message.client.commands
        .get('editSpecial')
        .execute(message, currUserData, image, hehe);
    } else {
    ...
    collector.on('collect', async (interaction) => {
      ...
      if (interaction.customId == 'back' || interaction.customId == 'reset') {
        ...
        try {
          ...
          console.log(
            'Error refreshing card image when back button pressed: ' + e.message
          );
        }
      ...
      if (interaction.customId.startsWith('fg:')) {
        for (let i = 0; i < achArray.length; i++) {
          if (interaction.customId == `fg:${i}`) {
            ...
            const newcanvas = await generateCard(currUserData, message);
            const newattach = await new AttachmentBuilder(
              await newcanvas.encode('png'),
              { name: 'card.png' }
            );
            msg.edit({
              files: [newattach],
        ...
        for (let i = 0; i < achArray.length; i++) {
          if (interaction.customId == `bg:${i}`) {
            ...
            try {
              ...
              console.log(
                'Error editing currUserData in editcard: ' + e.message
              );
            }
            const newcanvas = await generateCard(currUserData, message);
            const newattach = await new AttachmentBuilder(
        ...
        const patBackButton = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId('patreon')
            .setLabel('back to menu')
            .setStyle(ButtonStyle.Secondary)
